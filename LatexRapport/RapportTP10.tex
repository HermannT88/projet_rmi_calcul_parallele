...
\author{CERDA DE ALMEIDA VILACA Alexis\\ FOUSSE Emelyne \\ SAGET Logan \\ HERMANN Taïno \\ }
\date{Juin 2026}
...
\begin{itemize}
    \item Les nœuds de calcul (workers) : Ils démarrent, s'enregistrent auprès du service central, et exposent leurs méthodes de calcul sur des ports dynamiques. Ils peuvent se déconnecter ou tomber en panne à tout moment.
    \item Le client (dispatcher) : Il se connecte de manière éphémère lorsqu'il a une image à calculer. Il interroge le service pour obtenir les nœuds, effectue sa distribution de tâches en parallèle, puis se termine une fois l'image finale assemblée.
\end{itemize}
...
\begin{itemize}
    ...
    \item Entre le Client et le Service Central : Le client demande à récupérer des nœuds de calcul. Le service lui retourne en réponse la référence distante (stub) d'un nœud disponible en appliquant une politique de round-robin.
